...
export type NextCommitmentGuidance = {
  goal: string;
  example: string;
};

/**
 * Stage guidance is a playbook, not a recorded promise. The example helps the
 * seller write one concrete next action without manufacturing buyer agreement.
 */
export function nextCommitmentGuidance(stage: DealStage): NextCommitmentGuidance {
  switch (stage) {
    case "new":
      return {
        goal: "Decide whether this opportunity deserves a discovery investment.",
        example: "Wahala reviews the opportunity and records pursue or decline.",
      };
    case "discovery":
      return {
        goal: "Secure the next piece of evidence or stakeholder access needed to qualify and scope responsibly.",
        example: "Client sends two representative workflow samples for Wahala’s scope review.",
      };
    case "proposal_out":
      return {
        goal: "Get a dated buyer response—approval, decline, or specific objections—not an open-ended follow-up.",
        example: "Client reviews the proposal and returns approval or named objections.",
      };
    case "negotiating":
      return {
        goal: "Assign the next unresolved commercial term to one party so negotiation keeps moving.",
        example: "Wahala sends revised IP language for client review.",
      };
    case "committed":
      return {
        goal: "Complete the next agreement or payment gate required before delivery begins.",
        example: "Client signs the commercial agreement.",
      };
    case "won":
      return {
        goal: "Sales commitments are complete; delivery now owns the next project action.",
        example: "Wahala schedules the project kickoff in the Project record.",
      };
    case "lost":
      return {
        goal: "No active sales commitment remains; preserve the loss reason and any future re-entry condition.",
        example: "No next sales action unless the buyer agrees to reopen the opportunity.",
      };
  }
}
