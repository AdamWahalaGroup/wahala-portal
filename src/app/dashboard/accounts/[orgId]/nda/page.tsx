/**
 * NDA document — the account-level mutual NDA, boilerplate
 * (docs/Wahala_Group_Standard_Mutual_NDA.docx v1.0 → src/domain/nda.ts)
 * auto-populated from live account data. Reached from the deal's agreement
 * package (NDA row seeds at Committed) and the Account page's Agreements rail.
...
import { ndaMarkdown, NDA_TEMPLATE_VERSION } from "@/domain/nda";
...
export default async function NdaPage({ params }: { params: Promise<{ orgId: string }> }) {
  const ctx = await getAuthContext();
  if (!ctx) redirect(LOGIN_PATH);
  if (!ctx.isStaff) redirect("/dashboard");

  const { orgId } = await params;
  const view = await agreementDocViewFor(ctx, orgId, "nda");
  if (!view) notFound();
  ...
  return (
    ...
    >
      <div className="mono no-print" style={{ fontSize: 11, color: "var(--muted-line)", marginBottom: 14 }}>
        ...
        <Link href={`/dashboard/accounts/${orgId}`} style={{ color: "inherit" }}>{view.orgName}</Link> / NDA
      </div>
      <MsaDoc
        kind="nda"
        md={md}
