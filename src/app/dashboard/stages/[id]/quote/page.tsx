...
export default async function QuoteBuilderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (stage.status !== "draft") redirect(`/dashboard/stages/${id}`);

  // Quote authoring is admin or the org's Account Owner only.
  const isOwner = ctx.user.id === resource.accountOwnerUserId;
  if (!(ctx.isAdmin || (ctx.user.role === "account_owner" && isOwner))) redirect(`/dashboard/stages/${id}`);
  ...
  return (
    ...
    >
      <div className="mono" style={{ fontSize: 12, color: "var(--muted)" }}>
        ...
        <Link href={`/dashboard/stages/${stage.id}`}>{stage.name}</Link> / Build quote
      </div>
