# Delivery — phases, quote, tasks

> Part of the **Wahala Portal** design handoff — see [handoff index](../README.md). Visual reference: the labeled frames in `../Wahala Portal.dc.html`. **Naming:** this doc uses **Phase** for the project delivery unit (the old "Stage" name) — **Stage** is now reserved for the deal pipeline, see `CRM-RESTRUCTURE.md`.

### 05 — Phase detail (`/dashboard/phases/:id`) — THE KEY SCREEN
- **Purpose:** Everything about one phase; where quote→approve→pay→deliver→accept
...
- **Main:** title + StatusBadge; **Stepper**; a celebrated **Paid banner**
  ...
  the phase is one fixed price — **per-deliverable prices and estimate notes are
...
- **Right rail:** big tabular **Phase price** (fixed) + paid sub-line; **Your next
...
- **Purpose:** Set a phase's **fixed price** and its **deliverables** (grouped by
...
- **Layout:** header (title + Draft badge); 2-col: editor + 320px summary rail.
  **Editor is epic-grouped:** each **epic** is a card with an editable epic-name
  ...
  editable **Phase price · fixed** `$` field (hint: "{N} epics · {N} deliverables ·
...
### 09 — Tasks (delivery)
- **Purpose:** A phase's tasks with status + assignee; client "on you" items;
