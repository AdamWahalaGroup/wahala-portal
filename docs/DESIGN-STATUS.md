...
| 11 | Messages / comms | ✅ | `/dashboard/messages` | Built. **Two thread levels:** an **account thread** per client org (durable client↔Wahala line, exists before any project) + one per **project**. 300px thread list (Account tag, waiting-on dot, snippet, org for staff) + thread view — header **WaitingOn pill** (phrased for the viewer), bubbles (Wahala left `#F4F5F7` / client right `#EEF0FE`, avatar + name + mono "{org} · {date time}"), composer (input + "Needs a reply from {them}" flag + Ink Send, ⌘/Ctrl-Enter). Auto-refreshes. Tenant- + visibility-scoped (clients never see internal; cross-tenant post → 404). Threads keyed `account:<org>` / `project:<id>`. **Per-stage messaging is intentionally NOT a separate inbox** — stage discussion will live inline on the stage (Phase 2). |
