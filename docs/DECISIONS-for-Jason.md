...
| Auth | **Clerk** (Organizations = clients) | TDD was hand-rolled JWT. |
...
Stand up the **Phase 0 Cloudflare skeleton** (Next.js/OpenNext + D1 + Clerk + R2 + Stripe test mode), then build the **Phase 1 core loop**: onboard a client → assign an owner → client account with files/history → quote a stage → pay → deliver → accept. Meeting-digestion AI comes in Phase 2.
